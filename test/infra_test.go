...
// Deploy Dockerized Gin App to Koyeb
// Provision a database on Koyeb
// Create a Request to Health Endpoint of the Koyeb App and assert the response
// Create a request to create a feed and assert the response
// Tear down the infrastructure

func TestKoyebApp(t *testing.T) {
	cwd, err := os.Getwd()
	if err != nil {
		t.FailNow()
	}
	...
	test := integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
			...
			// })
			// assert health check
			assertHTTPResultShapeWithRetry(t, "http://"+stack.Outputs["app_url"].(string)+"/api/v1/health", nil, time.Minute,
				func(body string) bool { return assert.Contains(t, body, "message") },
				func(body string) bool { return assert.Contains(t, body, "OK") })
